...
// v4.1 — formato de saída no prompt, parse tolerante, resultado inválido
ok(sys.includes("FORMATO DE SAÍDA")&&sys.includes('"i1"')&&sys.includes('"1a"')&&sys.includes('"score": "100" | "50" | "0"')&&sys.includes('"evidencia"')&&sys.includes('"alergias_documentadas"')&&sys.includes('"termo"'),"system P: template de saída com as chaves reais");
ok(sys.includes('"1b"') && /"1b": \{[^}]*"score": "100" \| "50" \| "0" \| "NA"/s.test(sys) && !/"1a": \{[^}]*"NA"/s.test(sys),"template: NA só em critérios que aceitam");
ok(m.buildSystemA().includes('"i3"')&&m.buildSystemA().includes('"C"'),"system A: template de saída");
const sysNoI2=m.buildSystemP(false); ok(!sysNoI2.slice(sysNoI2.indexOf("FORMATO DE SAÍDA")).includes('"i2": {')&&sysNoI2.includes('"i1": {'),"template sem I2 quando não avaliável");
const tpl=m.schemaToTemplate(m.buildSchemaA()); ok(tpl.startsWith("{")&&tpl.trim().endsWith("}"),"template é um objeto");
ok(m.parseModelJson('{"a":1}').a===1,"parse: JSON puro");
ok(m.parseModelJson('```json\n{"a":2}\n```').a===2,"parse: JSON entre crases");
ok(m.parseModelJson('Aqui está a avaliação:\n{"a":3}\nEspero ter ajudado.').a===3,"parse: JSON com texto em volta");
let threw=false; try{m.parseModelJson("sem json aqui");}catch{threw=true;} ok(threw,"parse: erro claro sem JSON");
const blankMap=Object.fromEntries(m.I1_ALL.map(c=>[c.k,{score:""}])); blankMap["3a"].score="0";
ok(m.blankRatio([blankMap])>0.5,"blankRatio: 8/9 em branco → inválido");
